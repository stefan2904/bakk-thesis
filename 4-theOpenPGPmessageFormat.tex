...
In this section we give an overview of the basic building blocks of openPGP called \textit{Packets}\todo{RFC4880}. We describe how openPGP organizes data on it's lowest level. We list and describe some important Packets. Furthermore we show the structure of a Packet by example.  \\
...
OpenPGP currently defines 17 different Packet types. The following paragraph gives an overview of the important Packet types. A detailed description can be found in \todo{RFC4880}. \\ 
...
OpenPGP defines four different Transferable types \todo{RFC4880}. \\
...
To process the actual Transferable a OpenPGP implementation first removes the header and footer. Afterward a base64 parser is used to recover the Packet bytes while calculating the crc24 checksum. Finally a (optional) comparison with the appended checksum is performed.
